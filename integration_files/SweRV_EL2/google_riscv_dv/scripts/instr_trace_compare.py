  ...
  with open(csv1, "r") as fd1, open(csv2, "r") as fd2:
    ...
    if in_order_mode:
      ...
      for trace in instr_trace_1:
        ...
        instr1, instr2 = trace.binary.strip().zfill(8), instr_trace_2[trace_2_index].binary.strip().zfill(8)
        while (instr1 != instr2 or trace_2_index < trace_1_index):
          gpr_state_change_2 = check_update_gpr(instr_trace_2[trace_2_index].gpr,
                                                gpr_val_2)
          trace_2_index += 1
          instr1, instr2 = trace.binary.strip().zfill(8), instr_trace_2[trace_2_index-1].binary.strip().zfill(8)
        ...
        if gpr_state_change_2 == 0:
          ...
          found_mismatch = 0
          gpr1, gpr2 = str(trace.gpr).strip(), str(instr_trace_2[trace_2_index-1].gpr).strip()
          instr1, instr2 = trace.binary.strip().zfill(8), instr_trace_2[trace_2_index-1].binary.strip().zfill(8)
          if gpr1 != gpr2:
            x = gpr2.find(":deadbeef")
            if x > 0:	
              gpr2 = gpr1
          if gpr1 != gpr2 or instr1 != instr2:
            mismatch_cnt += 1
            found_mismatch = 1
            # print first few mismatches
            if mismatch_cnt <= mismatch_print_limit:
