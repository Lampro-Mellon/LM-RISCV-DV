...
def rtl_sim(sim_cmd, test_list, seed_gen, opts,
    ...
    for test in test_list:
        for i in range(test['iterations']):
        	...
        	#print("Command: " + cmd[1] + "\n")
        	run_cmd(cmd[1], 360, check_return_code=True)
...
def compare_test_run(test, idx, iss, output_dir, report):
    ...
    trace_log = os.path.join(rtl_dir, 'trace_00000000.log')
    out_res = os.path.join(rtl_dir, 't.log')
    ...
    try:
        # my own function 
        valid_log(trace_log, rtl_log_f,out_res)
    except RuntimeError as e:
        with open(report, 'a') as report_fd:
    ...
    try:
        ...
        process_core_sim_log(out_res, rtl_csv, 1)
    except RuntimeError as e:
        with open(report, 'a') as report_fd:
            report_fd.write('Log processing failed: {}\n'.format(e))
