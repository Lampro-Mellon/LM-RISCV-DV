...
def gcc_compile(test_list, output_dir, isa, mabi, opts, gcc_user_extension_path, linker_path, debug_cmd):
    ...
    for test in test_list:
        for i in range(0, test['iterations']):
            ...
            binary = prefix + ".bin"
            dump = prefix + ".dump"
            program_hex = ("{}/asm_tests/program_{}_{}.hex" .format(output_dir, test['test'], i))
            test_isa = isa
            if not os.path.isfile(asm) and not debug_cmd:
                logging.error("Cannot find assembly test: {}\n".format(asm))
                sys.exit(RET_FAIL)
            test_name = test.get('test')
            cust_linker_path = "riscv_dv_extension/linker_scripts/{}.ld" .format(test_name)
            ...
            if os.path.exists(cust_linker_path):
                linker_path = cust_linker_path
            cmd = ("{} -m elf32lriscv --discard-none -T{} -o {} {}" .format(get_env_var("RISCV_LD", debug_cmd = debug_cmd), linker_path, exe, elf))
            run_cmd_output(cmd.split(), debug_cmd = debug_cmd)
            
            # Generating disassembly dump
            cmd = ('{} --disassemble-all --disassemble-zeroes -M no-aliases --section=.text --section=.data {} > {}' .format(get_env_var("RISCV_OBJDUMP", debug_cmd = debug_cmd), elf, dump))
            os.system(cmd)
            
            # Generating Program.hex
            cmd = ('{} -O verilog {} {}' .format(get_env_var("RISCV_OBJCOPY", debug_cmd = debug_cmd), exe, program_hex))
            os.system(cmd)


def run_assembly(asm_test, iss_yaml, isa, mabi, gcc_opts, iss_opts, output_dir, gcc_user_extension_path, linker_path,
   ...
   # report = ("{}/iss_regr.log".format(output_dir)).rstrip()
    ...
    binary = prefix + ".bin"
    dump = prefix + ".dump"
    program_hex = ("{}/directed_asm_tests/program_{}.hex" .format(output_dir, asm))
    # TODO (Haroon): Enable it after setting up spike simulation for directed asm tests
    ...
    cust_linker_path = Path("directed_tests/asm/{}.ld" .format(test))
    ...
    if asm_test.endswith(".S"):
        ...
        cmd = ("{} -static -mcmodel=medany \
            ...
            get_env_var("RISCV_GCC", debug_cmd=debug_cmd), asm_test, gcc_user_extension_path, linker_path, gcc_opts, elf))
        cmd += (" -march={}".format(isa))
        cmd += (" -mabi={}".format(mabi))
        run_cmd_output(cmd.split(), debug_cmd=debug_cmd)
    elif asm_test.endswith(".s"):
    #TODO (Najeeb do linked elf gen for spike)
      cmd = ("{}/bin/riscv64-unknown-elf-cpp -I{} {} > {}" .format(get_env_var("RISCV_TOOLCHAIN", debug_cmd = debug_cmd), gcc_user_extension_path, asm_test, cpp_s))
      os.system(cmd)
      cmd = ("{}/bin/riscv64-unknown-elf-as -march={} {} -o {}" .format(get_env_var("RISCV_TOOLCHAIN", debug_cmd = debug_cmd), isa, cpp_s, elf))
      os.system(cmd)
        
    # Convert the ELF to plain binary and hex
    # Generating Binary
    cmd = ("{} -O binary {} {}" .format(get_env_var("RISCV_OBJCOPY", debug_cmd = debug_cmd), elf, binary))
    run_cmd_output(cmd.split(), debug_cmd = debug_cmd)
    ...
    # Generating disassembly dump
    cmd = ('{} --disassemble-all --disassemble-zeroes -M no-aliases {} > {}' .format(get_env_var("RISCV_OBJDUMP", debug_cmd = debug_cmd), elf, dump))
    os.system(cmd)
...
def run_assembly_from_dir(asm_test_dir, iss_yaml, isa, mabi, gcc_opts, iss,
    ...
    Args:
      ...
      debug_cmd       : Produce the debug cmd log without running
    """
    result = run_cmd("find {} -name \"*.S\" -o -name \"*.s\"".format(asm_test_dir))
    test_name = test_entry.get('test')
    cust_linker_path = Path("directed_tests/asm/{}.ld" .format(test_name))
    if result:
        asm_list = result.splitlines()
        logging.info("Found {} assembly tests under {}".format(
            len(asm_list), asm_tests_dir))
...
def run_c(c_test, iss_yaml, isa, mabi, gcc_opts, iss_opts, output_dir,
    ...
    exe = prefix + ".exe"
    dump = prefix + ".dump"
    program_hex = ("{}/directed_c_tests/program_{}.hex" .format(output_dir, c))
    cust_linker_path = Path("directed_tests/c/{}.ld" .format(test))
     # TODO (Haroon): Enable it after setting up spike simulation for directed c tests
    #iss_list = iss_opts.split(",")
    ...
    if os.path.exists(cust_linker_path):
      linker_path = cust_linker_path
    cmd = ("{} -I{} -T{} -nostdlib -nostartfiles {} -march={} -mabi={} {} -o {} ".format(get_env_var("RISCV_GCC", debug_cmd = debug_cmd), gcc_user_extension_path, linker_path, gcc_opts, isa, mabi, c_test, elf))
    run_cmd_output(cmd.split(), debug_cmd = debug_cmd)
        ...
        if 'no_iss' in test and test['no_iss'] == 1:
          ...
          for i in range(0, test['iterations']):
            if "asm_tests" in test.keys():
              prefix = ("{}/directed_asm_tests/{}" .format(output_dir, test['test']))
            elif "c_tests" in test.keys():
              prefix = ("{}/directed_c_tests/{}" .format(output_dir, test['test']))
            else:
    ...
    try:
            ...
            for path_asm_test in asm_test:
                ...
                # path_asm_test is an assembly file
                    ...
                    run_assembly(full_path, args.iss_yaml, args.isa, args.mabi,
                                 args.gcc_opts,
                                 args.iss, output_dir,  args.gcc_user_extension_path, args.linker_path, args.core_setting_dir,
                                 args.debug)
                else:
        ...
        if not args.co:
            ...
            for t in list(matched_list):
                # Check mutual exclusive between gen_test, asm_tests, and c_test
                if 'asm_tests' in t:
                    ...
                    if path_asm_test:
                        ...
                        if os.path.isdir(path_asm_test):
                            run_assembly_from_dir(path_asm_test, args.iss_yaml,
                                                  args.isa, args.mabi,
                                                  gcc_opts, args.iss,
                                                  output_dir, args.gcc_user_extension_path, args.linker_path,
                        ...
                        # path_asm_test is an assembly file
                            ...
                            run_assembly(path_asm_test, args.iss_yaml, args.isa,
                                         args.mabi, gcc_opts,
                                         args.iss, output_dir, args.gcc_user_extension_path, args.linker_path,
                                         args.core_setting_dir, args.debug)
                        else:
                            if not args.debug:
                                logging.error(
