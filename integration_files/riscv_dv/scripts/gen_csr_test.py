...
def predict_csr_val(csr_op, rs1_val, csr_val, csr_address, csr_write_mask, csr_read_mask):
  ...
  if csr_op == 'csrrw':
    ...
    #if  csr_address == 803:
      prediction = bitarray(hex=f"0x{'00000032'}")
    #else:
    #if  ((csr_val[5] & csr_val[4] & (csr_val[3]|csr_val[2]|csr_val[1])) | ((csr_val[31])|(csr_val[30])|(csr_val[29])|(csr_val[28])|(csr_val[27])|
     #  (csr_val[26])|(csr_val[25])|(csr_val[24])|(csr_val[23])|(csr_val[22])|(csr_val[21])|(csr_val[20])|(csr_val[19])|(csr_val[18])|
      # (csr_val[17])|(csr_val[16])|(csr_val[15])|(csr_val[14])|(csr_val[13])|(csr_val[12])|(csr_val[11])|(csr_val[10])|(csr_val[9])|
       #(csr_val[8])|(csr_val[7])|(csr_val[6]))):
    #if  csr_address == 803:
      #prediction = bitarray(hex=f"0x{'0000001B'}")
    else:
          ...
          for i in range(3):
            ...
            if op[-1] == "i":
              ...
              predict_li = (f"\tli {source_reg}, "
                f"{predict_csr_val(op, imm_val, csr_val, csr_address, csr_write_mask, csr_read_mask)}\n")
            else: 
              first_li = f"\tli {source_reg}, 0x{rand_rs1_val.hex}\n"
              csr_inst = f"\t{op} {dest_reg}, {csr_address}, {source_reg}\n"
              nop = f"\tnop\n"
              predict_li = (f"\tli {source_reg}, "
                f"{predict_csr_val(op, rand_rs1_val, csr_val, csr_address, csr_write_mask, csr_read_mask)}\n")
           ...
           # csr_test_file.write(nop)
            ...
            if csr == csr_list[-1] and op == csr_instructions[-1] and i == 2:
              ...
              final_li = (f"\tli {source_reg}, "
                f"{predict_csr_val('csrrs', csrrs_read_mask, csr_val, csr_address, csr_write_mask, csr_read_mask)}\n")
