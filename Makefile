...
GEN_DIR             := $(realpath ../../google_riscv_dv/)
TOOLCHAIN           := ${RISCV_TOOLCHAIN}

# Explicitly ask for the bash shell
SHELL                := bash

# The number of instrucctons to be removed (by default zero means all the instruction added)
Count                := 0	
...
GEN_OPTS            :=
# Compile time options for core RTL simulation
COMPILE_OPTS        +=
# Run time options for core RTL simulation
SIM_OPTS            :=
...
# Linker File Flag if 1 use custom linker else use linker file from google risc-v dv 
LINK				= 1
# provide specific link file
ifeq (,$(wildcard riscv_dv_extension/$(TEST).ld))
	# path to custom linker file
	LINK_FILE	= riscv_dv_extension/link.ld 
else
	LINK_FILE = riscv_dv_extension/$(TEST).ld
endif
...
sim: Instr_adder compile gen gcc_compile rtl_sim cov_urg_test
...
clean:
	...
	rm -rf out/
	rm -rf tests_cov/
	rm ucli.key novas* program.hex program.bin data.hex *.log trace_port.csv
...
#--------------------------------------------------------------------------
#This target is used to remove the instruction from the list of unsupported instruction
#by pyhton script
Instr_adder:
	python3 riscv_dv_extension/instr_adder_removr.py $(Count)
###############################################################################
...
link-arg 		:= $(if $(call equal,$(LINK),1),--linker_path=$(realpath $(LINK_FILE)),)
...
$(metadata)/instr_gen.gen.stamp: \
  $(gen-vars-prereq) $(risc-dv-files) $(TESTLIST) | $(metadata)
	...
	$(verb)python3 ${GEN_DIR}/lm_run.py \
     --output=$(OUT-SEED)/instr_gen ${GEN_OPTS} \
     --steps=gen \
     --gen_timeout=${TIMEOUT} \
     --lsf_cmd="${LSF_CMD}" \
     --simulator="${SIMULATOR}" \
     --gcc_user_extension_path=${GCC_USER_EXT_PATH} \
     $(link-arg) \
     ${RISCV_DV_OPTS} \
     ${TEST_OPTS} \
     ${CSR_OPTS} \
     --sim_opts="+signature_addr=${SIGNATURE_ADDR} +tvec_alignment=8"
...
#
# The compiled testbench (obviously!) also depends on the design and the
...
# .vh etc file in the testbench or rtl directories. Note that this variable is set with
# '=', rather than ':='. This means that we don't bother running the find
# commands unless we need the compiled testbench.
all-src-files = $(shell find rtl -name '*.v' -o -name '*.sv' -o -name '*.vh' -o -name '*.svh') \
  				$(shell find testbench -name '*.v' -o -name '*.sv' -o -name '*.vh' -o -name '*.svh')
...
$(OUT)/rtl_sim/.compile.stamp: \
  $(compile-vars-prereq) $(all-src-files) $(risc-dv-files) \
...
###############################################################################
# Run Core RTL simulation with generated programs
#
...
cov_urg_all:
	urg -lca -dir $(shell find ${OUT} -type d -name "test.vdb" -not -path "./${OUT}/rtl_sim/*")
	
# Generate urg report for a specific seed
cov_urg_test:
	urg -lca -dir "./$(OUT-SEED)/rtl_sim/test.vdb" -report "tests_cov/$(TEST)/$(SEED)/"
