...
# TODO: (saqib) the cmd might need to be changed for the cores
...
def run_c(c_test, iss_yaml, isa, mabi, gcc_opts, iss_opts, output_dir,
  ...
  L_path = ("%s/install/lib/release"  % (gcc_user_extension_path))
  I_path = ("%s/install/include"  % (gcc_user_extension_path))
  cmd = ("%s -I%s --specs=nano.specs -DMTIME_RATE_HZ_DEF=32768 -T%s -nostdlib -nostartfiles -Os  -Wl,--gc-sections -Wl,-Map,%s.map\
         -march=%s -mabi=%s -mcmodel=medlow -ffunction-sections \
         -fdata-sections -L%s %s -Wl,--start-group -lc -lgcc -lm -lmetal -lmetal-gloss -Wl,--end-group -o %s " % \
          (get_env_var("RISCV_GCC", debug_cmd = debug_cmd), I_path, linker_path, c_test, isa, mabi, L_path, c_test, elf))
  run_cmd_output(cmd.split(), debug_cmd = debug_cmd)
